...
function getUsers(userIds, done) {
  // TODO: Implement.
  let usersPromisified = [];

  for (let i = 0; i < userIds.length; i++) {
    usersPromisified.push(
      new Promise((resolve, reject)=>{
        getUser(userIds[i], resolve); 
      })
    );
  }

  Promise.all(usersPromisified).then((usersResolved) => {
  	done(usersResolved);
  }).catch((err)=>{
    console.err("Err in promisifying users:", err);
  });
}
